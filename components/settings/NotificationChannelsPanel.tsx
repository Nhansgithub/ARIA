...
export function NotificationChannelsPanel() {
  ...
  useEffect(() => {
    if (showEmailOffDialog) {
      dialogCancelRef.current?.focus()
    } else {
      ;(dialogTriggerRef.current as HTMLElement | null)?.focus()
    }
  }, [showEmailOffDialog])
  ...
  function closeEmailDialog() {
    setShowEmailOffDialog(false)
  }
  function confirmEmailOff() {
    setShowEmailOffDialog(false)
    persistPatch({ email_enabled: false })
  }
    ...
    >
      ...
      {loading ? (
        ...
        <>
          ...
          <ChannelRow
            title="In-app"
            description="Thông báo trong ứng dụng ARIA — kênh chính, luôn hoạt động."
          >
            <span
              style={{
                fontSize: 12,
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span
                style={{
                  fontSize: 12,
                  fontWeight: 600,
                  color: zaloStatusColor,
                  background: `${zaloStatusColor}22`,
                  borderRadius: 4,
                  padding: '3px 8px',
                }}
              >
          ...
          <ChannelRow
            title="Email"
            description="Nhận thông báo qua email dự phòng khi Zalo không khả dụng."
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              {saveStatus === 'saved' && (
                <span aria-hidden="true" style={{ fontSize: 13, color: '#14b8a6' }}>
                  Đã lưu
                </span>
              )}
              {saveStatus === 'error' && (
                <span aria-hidden="true" style={{ fontSize: 13, color: '#ef4444' }}>
                  Lỗi — thử lại
                </span>
              )}
              <Toggle
      ...
      {showZaloWizard && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Kết nối Zalo OA"
          onKeyDown={(e) => {
            if (e.key === 'Escape') closeZaloWizard()
          }}
        >
          <div
            style={{
          ...
          >
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              >
                ...
                <span style={{ fontSize: 13, color: '#94a3b8', lineHeight: 1.5 }}>
                  ...
                  Tôi đã mở app Zalo, tìm trang OA &quot;
                  {process.env.NEXT_PUBLIC_ZALO_OA_NAME ?? 'ARIA'}&quot; và nhấn{' '}
                  <strong style={{ color: '#14b8a6' }}>Quan tâm</strong> (Follow).
                </span>
              </label>
            </div>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button
                ...
                style={{
                  fontSize: 14,
                  color: canConnectZalo ? '#0a0e27' : '#64748b',
                  background: canConnectZalo ? '#14b8a6' : '#1e2a45',
                  border: 'none',
                  borderRadius: 6,
                  padding: '7px 16px',
                  cursor: canConnectZalo ? 'pointer' : 'not-allowed',
                  fontWeight: 600,
                }}
              >
      ...
      {showEmailOffDialog && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Xác nhận tắt email"
          onKeyDown={(e) => {
            if (e.key === 'Escape') closeEmailDialog()
          }}
        >
          <div
            style={{
          ...
          >
            <p style={{ fontSize: 15, lineHeight: 1.7, marginBottom: 20 }}>
              Nếu tắt email và Zalo chưa kết nối, anh sẽ chỉ thấy thông báo trong app. Tiếp tục
              không?
            </p>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <button
  ...
  return (
    <button
      ...
      style={{
        ...
        height: 24,
        borderRadius: 12,
        background: checked ? '#14b8a6' : '#2a3350',
        border: 'none',
