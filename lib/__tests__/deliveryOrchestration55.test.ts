...
test('T1: email cron file exists', () => assert(exists('app/api/cron/send-emails/route.ts'), 'missing'))
test('T2: briefing query skips zalo_status=sent (neq filter)', () => {
  const code = codeLines(emailCronRoute).join('\n')
  assert(
    (code.includes("neq('zalo_status'") || code.includes('neq("zalo_status"')) && code.includes("'sent'"),
    'briefings query must filter out zalo_status=sent'
  )
})
test('T3: check-in query skips zalo_status=sent (neq filter)', () => {
  const code = codeLines(emailCronRoute).join('\n')
  const neqCount = (code.match(/neq.*zalo_status/g) ?? []).length + (code.match(/zalo_status.*neq/g) ?? []).length
...
test('T4: email still sends when zalo_status=pending', () => {
  const code = codeLines(emailCronRoute).join('\n')
  assertNotIncludes(code, "eq('zalo_status', 'pending')", 'must not filter TO pending — only filter OUT sent')
})
...
test('T7: briefings select includes email_sent_at null guard', () => assertIncludes(emailCronRoute, 'email_sent_at'))
...
test('T11: pg_cron email send migration exists', () => assert(exists('supabase/migrations/20260701500000_pg_cron_email_send.sql'), 'missing'))
test('T12: email cron fires at :15 UTC', () => assertIncludes(pgCronEmailMig, '15 * * * *'))
test('T13: Zalo cron fires at :05 UTC (before email)', () => assertIncludes(pgCronZaloMig, '5 * * * *'))
test('T14: email cron calls /api/cron/send-emails', () => assertIncludes(pgCronEmailMig, '/api/cron/send-emails'))
test('T15: email cron uses cron_secret', () => assertIncludes(pgCronEmailMig, 'cron_secret'))
test('T16: email cron migration uses unschedule guard', () => assertIncludes(pgCronEmailMig, 'unschedule'))
...
test('T18: email cron has timeout_milliseconds set', () => assertIncludes(pgCronEmailMig, 'timeout_milliseconds'))
...
test('T20: both cron schedules are per-hour not per-day', () => {
  assert(pgCronZaloMig.includes('* * * *') && pgCronEmailMig.includes('* * * *'), 'both must be hourly crons')
})

// ─── T21-T30: Zalo cron — sends first, DB state as handshake ────────────────
console.log('── Zalo cron: sends first, sets zalo_status=sent')

test('T21: Zalo send cron file exists', () => assert(exists('app/api/cron/send-zalo/route.ts'), 'missing'))
...
test('T25: Zalo cron uses timing-safe CRON_SECRET comparison', () => {
  ...
  assert(importLines.some(l => l.includes('timingSafeEqual') || l.includes('crypto')), 'must use timingSafeEqual')
})
test('T26: Zalo cron filters briefings by zalo_status=pending', () => assertIncludes(zaloCronRoute, "'pending'"))
test('T27: Zalo cron checks DB update errors (P1 from review)', () => {
  const code = codeLines(zaloCronRoute).join('\n')
  assert(code.includes('updateErr') || code.includes('error: update'), 'update errors must be checked')
})
...
test('T32: email fallback when Zalo fails (failed ≠ sent → email still runs)', () => {
  const code = codeLines(emailCronRoute).join('\n')
  assert(
    !code.includes("eq('zalo_status', 'failed'") ||
    code.includes("neq('zalo_status', 'sent'"),
...
test('T34: delivery order documented in email cron (AD-8 comment or story comment)', () => {
  ...
  assert(code.includes('zalo') && (code.includes('ad-8') || code.includes('skip') || code.includes('priority')), 'orchestration comment expected in email cron')
})
...
test('T37: zalo_status column exists in briefings migration (Story 5.4)', () => {
  assert(exists('supabase/migrations/20260701400000_zalo_delivery_status.sql'), '5.4 migration missing')
...
test('T40: main test chain includes deliveryOrchestration55', () => {
  ...
  assert(main !== undefined && main.includes('deliveryOrchestration55'), 'missing from main test chain')
})
...
test('T44: Email cron has no Zalo dependency — decoupled via DB state only', () => {
  ...
  assertNotIncludes(emailCronRoute, 'zaloTokenService', 'email cron must not import Zalo token service')
})
