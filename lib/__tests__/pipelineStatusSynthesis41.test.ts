...
{
  const result = synthesizeDeal({ id: 'd1', title: 'Phở 24 Website', stage: 'proposal', service_type: 'web_design', value_estimate: 50_000_000, days_idle: 4 })
...
{
  const result = synthesizeDeal({ id: 'd2', title: 'ACME Deal', stage: 'đang ở giai đoạn đề xuất', service_type: 'web_design', value_estimate: 30_000_000, days_idle: 2 })
...
console.log('\nT3: Contract stage → push_for_signature')
{
  const result = synthesizeDeal({ id: 'd3', title: 'Tech Corp App', stage: 'contract review', service_type: 'web_app', value_estimate: 200_000_000, days_idle: 1 })
...
console.log('\nT4: Discovery stage → schedule_discovery')
{
  const result = synthesizeDeal({ id: 'd4', title: 'New Lead', stage: 'discovery', service_type: 'automation', value_estimate: 0, days_idle: 0 })
...
console.log('\nT5: Kickoff stage → confirm_kickoff')
{
  const result = synthesizeDeal({ id: 'd5', title: 'ABC Project', stage: 'kickoff', service_type: 'web_design', value_estimate: 80_000_000, days_idle: 3 })
...
console.log('\nT6: Won stage → request_referral')
{
  const result = synthesizeDeal({ id: 'd6', title: 'BigCo Deal', stage: 'won', service_type: 'web_app', value_estimate: 300_000_000, days_idle: 0 })
...
{
  const result = synthesizeDeal({ id: 'd7', title: 'Weird Deal', stage: 'Đang chờ anh xem lại', service_type: 'other', value_estimate: 10_000_000, days_idle: 0 })
  assert(result.nextAction === 'contextual_inference', 'unrecognized free-text stage returns contextual_inference, not an error')
}

// T8: Deal idle 8 days → stale (>7 days)
console.log('\nT8: Deal idle 8 days → stale flag')
{
  const result = synthesizeDeal({ id: 'd8', title: 'Stale Deal', stage: 'proposal', service_type: 'web_design', value_estimate: 40_000_000, days_idle: 8 })
  assert(result.stale === true, '8 days idle is stale')
  assert(typeof result.staleMessage === 'string' && result.staleMessage.includes('8'), 'stale message mentions 8 days')
}

// T9: Deal idle exactly 7 days → stale (boundary: >=7)
console.log('\nT9: Deal idle exactly 7 days → stale (threshold is >=7)')
{
  const result = synthesizeDeal({ id: 'd9', title: 'Boundary Deal', stage: 'proposal', service_type: 'web_design', value_estimate: 40_000_000, days_idle: 7 })
  assert(result.stale === true, 'exactly 7 days is stale (threshold is >=7)')
  assert(typeof result.staleMessage === 'string' && result.staleMessage.includes('7'), 'stale message mentions 7 days')
}

// T10: Deal idle 0 days → not stale
console.log('\nT10: Deal idle 0 days → not stale')
{
  const result = synthesizeDeal({ id: 'd10', title: 'Fresh Deal', stage: 'discovery', service_type: 'web_app', value_estimate: 100_000_000, days_idle: 0 })
...
{
  const neg = synthesizeDeal({ id: 'd11a', title: 'Neg Deal', stage: 'negotiation', service_type: 'web_app', value_estimate: 150_000_000, days_idle: 2 })
  const sow = synthesizeDeal({ id: 'd11b', title: 'SOW Deal', stage: 'sow review', service_type: 'automation', value_estimate: 90_000_000, days_idle: 1 })
...
console.log('\nT12: orchestrator.ts has pipeline_status intent bucket')
{
  ...
  assert(content.includes("'pipeline_status'"), "orchestrator.ts includes 'pipeline_status' in IntentBucket or VALID_BUCKETS")
  assert(content.includes('PIPELINE STATUS PROTOCOL'), 'orchestrator.ts has PIPELINE STATUS PROTOCOL specialist prompt')
  assert(content.includes('pipeline_status: ARIA_MODELS.economical'), 'pipeline_status routes to economical (Haiku) model — AD-4')
}
...
console.log('\nT13: pipelineStatusTools.ts contains all four read-only tools with guards')
{
  ...
  assert(content.includes('get_pipeline_summary'), 'pipelineStatusTools references get_pipeline_summary')
  ...
  assert(content.includes('requireTool'), 'pipelineStatusTools uses requireTool guard (not ! assertions)')
  assert(!content.includes('find(')  || content.includes('requireTool'), 'no bare .find()! pattern — guarded access only')
}
...
console.log('\nT14: chat route handles pipeline_status intent')
{
  ...
  const content = fs.existsSync(src) ? fs.readFileSync(src, 'utf8') : ''
  assert(content.includes("classification.intent === 'pipeline_status'"), 'chat route routes pipeline_status')
...
console.log('\nT15: Stage keyword table does not include random Vietnamese text (confirms fallback)')
{
  ...
  assert(!allKeywords.includes('Đang chờ anh xem lại'), 'arbitrary Vietnamese free-text is not hardcoded — falls through to contextual_inference')
}
