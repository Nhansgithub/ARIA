...
test('T1: migration file exists', () =>
  assert(exists('supabase/migrations/20260701300000_zalo_setup.sql'), 'file missing'))
...
test('T4: adds zalo_token_issued_at column', () =>
  assertIncludes(migrationSetup, 'zalo_token_issued_at'))
...
test('T11: pg_cron migration file exists', () =>
  assert(exists('supabase/migrations/20260701310000_pg_cron_zalo_refresh.sql'), 'file missing'))
test('T12: schedules refresh-zalo-token cron job', () =>
  assertIncludes(migrationRefresh, 'refresh-zalo-token'))
test('T13: fires every hour at :05', () => assertIncludes(migrationRefresh, '5 * * * *'))
test('T14: calls /api/cron/refresh-zalo-token endpoint', () =>
  assertIncludes(migrationRefresh, '/api/cron/refresh-zalo-token'))
test('T15: uses cron_secret for authorization', () =>
  assertIncludes(migrationRefresh, 'cron_secret'))
...
test('T17: starts with import server-only (AD-11)', () => {
  ...
  assert(
    firstMeaningfulLine?.includes("import 'server-only'") ?? false,
    'must import server-only as first line'
  )
})
test('T18: exports isZaloConfigured function', () =>
  assertIncludes(tokenService, 'export function isZaloConfigured'))
test('T19: isZaloConfigured checks ZALO_APP_ID', () => assertIncludes(tokenService, 'ZALO_APP_ID'))
test('T20: isZaloConfigured checks ZALO_SECRET_KEY', () =>
  assertIncludes(tokenService, 'ZALO_SECRET_KEY'))
test('T21: exports exchangeCredentialsForTokens', () =>
  assertIncludes(tokenService, 'export async function exchangeCredentialsForTokens'))
test('T22: exports refreshAccessToken', () =>
  assertIncludes(tokenService, 'export async function refreshAccessToken'))
test('T23: uses AbortSignal.timeout for fetch (AD-6)', () =>
  assertIncludes(tokenService, 'AbortSignal.timeout'))
...
test('T25: uses Zalo token URL v4/oa', () =>
  assertIncludes(tokenService, 'oauth.zaloapp.com/v4/oa/access_token'))

// ─── T26-T35: connect route ───────────────────────────────────────────────────
console.log('── app/api/settings/zalo/connect/route.ts')

test('T26: file exists', () =>
  assert(exists('app/api/settings/zalo/connect/route.ts'), 'file missing'))
...
test('T31: calls exchangeCredentialsForTokens', () =>
  assertIncludes(connectRoute, 'exchangeCredentialsForTokens'))
...
test('T34: uses createServerClient (AD-13 owner path)', () => {
  ...
  assert(
    ...
    'must import createServerClient'
  )
})
test('T35: stores zalo_token_issued_at timestamp', () =>
  assertIncludes(connectRoute, 'zalo_token_issued_at'))

// ─── T36-T40: disconnect route ────────────────────────────────────────────────
console.log('── app/api/settings/zalo/disconnect/route.ts')

test('T36: file exists', () =>
  assert(exists('app/api/settings/zalo/disconnect/route.ts'), 'file missing'))
test('T37: nulls out zalo_user_id', () => assertIncludes(disconnectRoute, 'zalo_user_id: null'))
test('T38: nulls out zalo_access_token', () =>
  assertIncludes(disconnectRoute, 'zalo_access_token: null'))
test('T39: nulls out encrypted_zalo_refresh_token', () =>
  assertIncludes(disconnectRoute, 'encrypted_zalo_refresh_token: null'))
test('T40: sets zalo_status to not_configured', () =>
  assertIncludes(disconnectRoute, "'not_configured'"))

// ─── T41-T50: refresh-zalo-token cron route ───────────────────────────────────
console.log('── app/api/cron/refresh-zalo-token/route.ts')

test('T41: file exists', () =>
  assert(exists('app/api/cron/refresh-zalo-token/route.ts'), 'file missing'))
...
test('T44: uses createServiceClient (AD-13 cron path)', () => {
  ...
  assert(
    ...
    'must import createServiceClient'
  )
})
...
test('T46: queries settings by zalo_status=connected', () =>
  assertIncludes(cronRoute, "'connected'"))
test('T47: uses 50-minute refresh cutoff for 1-hour TTL', () => {
  const code = codeLines(cronRoute).join('\n')
  assert(
    code.includes('50') &&
      (code.includes('* 60') ||
        code.includes('* 60_000') ||
        code.includes('50 * 60') ||
        code.includes('50_')),
    '50-minute buffer expected'
  )
})
test('T48: calls refreshAccessToken on each owner', () =>
  assertIncludes(cronRoute, 'refreshAccessToken'))
test('T49: sets zalo_status=token_expired on refresh failure', () =>
  assertIncludes(cronRoute, "'token_expired'"))
...
test('T51: returns zalo_server_configured boolean', () =>
  assertIncludes(channelsRoute, 'zalo_server_configured'))
...
test('T56: PATCH rejects unknown fields with 400', () => {
  ...
  assert(
    code.includes('400') &&
      (code.includes('Unknown') || code.includes('OWNER_WRITABLE') || code.includes('read-only')),
    '400 for unknown fields expected'
  )
})
...
test('T57: component file exists', () =>
  assert(exists('components/settings/NotificationChannelsPanel.tsx'), 'file missing'))
test('T58: exports NotificationChannelsPanel', () =>
  assertIncludes(panel, 'export function NotificationChannelsPanel'))
test('T59: fetches /api/settings/notification-channels on mount', () =>
  assertIncludes(panel, '/api/settings/notification-channels'))
test('T60: shows Zalo setup wizard (showZaloWizard state)', () =>
  assertIncludes(panel, 'showZaloWizard'))
test('T61: wizard has zalo_user_id input', () => assertIncludes(panel, 'zaloUserId'))
test('T62: wizard has follow confirmation checkbox', () =>
  assertIncludes(panel, 'zaloFollowConfirmed'))
test('T63: POST to /api/settings/zalo/connect', () =>
  assertIncludes(panel, '/api/settings/zalo/connect'))
test('T64: disconnect button calls /api/settings/zalo/disconnect', () =>
  assertIncludes(panel, '/api/settings/zalo/disconnect'))
test('T65: shows server-not-configured message when zaloServerConfigured=false', () => {
  const code = codeLines(panel).join('\n')
  assert(
    code.includes('zaloServerConfigured') &&
      (code.includes('ZALO_APP_ID') ||
        code.includes('zalo-docs') ||
        code.includes('Chưa được cấu hình') ||
        code.includes('không được cấu hình') ||
        code.includes('Tính năng Zalo chưa')),
...
test('T70: main test chain includes zaloSetup53', () => {
  ...
  assert(
    mainTest !== undefined && mainTest.includes('zaloSetup53'),
    'main test chain must include zaloSetup53'
  )
})
