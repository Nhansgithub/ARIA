...
assert(
  stale7dIdx !== -1 && preActionIdx !== -1 && stale7dIdx !== preActionIdx,
...
assert(
  preActionIdx !== -1 && cadenceIdx !== -1 && preActionIdx !== cadenceIdx,
...
assert(
  pendingRouteContent.includes('export') && pendingRouteContent.includes('GET'),
  'T24: pending route exports GET'
)
...
assert(
  fs.existsSync(answerRoutePath),
  'T28: answer route exists at app/api/check-ins/[id]/answer/route.ts'
)

// T29: answer route contains createServerClient
assert(
  answerRouteContent.includes('createServerClient'),
  'T29: answer route contains createServerClient'
)

// T30: answer route exports a POST function
assert(
  answerRouteContent.includes('export') && answerRouteContent.includes('POST'),
  'T30: answer route exports POST'
)
...
assert(
  checkInCardContent.includes('#f59e0b'),
...
assert(
  chatPanelContent.includes('CheckInCardData'),
  'T47: ChatPanel.tsx imports CheckInCardData type'
)

// T48: ChatPanel.tsx contains pendingCheckIns state
assert(
  chatPanelContent.includes('pendingCheckIns'),
  'T48: ChatPanel.tsx contains pendingCheckIns state'
)
...
assert(
  chatPanelContent.includes('<CheckInCard'),
  'T51: ChatPanel.tsx renders CheckInCard components'
)
...
assert(
  chatPanelContent.includes('InputBar'),
  'T53: ChatPanel.tsx does NOT remove InputBar (InputBar preserved)'
)
...
assert(
  answerRouteContent.includes('{ value: answer }') ||
    answerRouteContent.includes('{ value: answer }'),
