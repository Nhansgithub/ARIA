...
{
  const b: BriefingRecord = { flags: { items: [{ severity: 'high', type: 'overdue' }, { severity: 'high', type: 'stale' }] }, seen_at: null }
...
{
  const b: BriefingRecord = { flags: { items: [{ severity: 'high', type: 'overdue' }] }, seen_at: '2026-07-01T05:00:00Z' }
  assert(computeBadgeCount(b, 0) === 0, 'T3: seen briefing does not contribute to badge')
}

// T4: Medium-severity flags don't count
{
  const b: BriefingRecord = { flags: { items: [{ severity: 'medium', type: 'stale' }, { severity: 'medium', type: 'missing_doc' }] }, seen_at: null }
...
// T5: Mix of high and medium — only high counted
{
  const b: BriefingRecord = {
    flags: { items: [{ severity: 'high', type: 'overdue' }, { severity: 'medium', type: 'stale' }, { severity: 'high', type: 'stale' }] },
    seen_at: null,
  }
...
{
  const b: BriefingRecord = { flags: { items: [{ severity: 'high', type: 'overdue' }] }, seen_at: null }
...
{
  const b: BriefingRecord = {
    flags: { items: Array.from({ length: 5 }, (_, i) => ({ severity: 'high' as const, type: `type${i}` })) },
    seen_at: null,
  }
...
{
  const b: BriefingRecord = {
    flags: { items: [{ severity: 'high', type: 'overdue' }, { severity: 'high', type: 'stale' }] },
...
const migrationPath = path.join(ROOT, 'supabase', 'migrations', '20260701000000_briefing_seen_at.sql')
const badgeCountRoutePath = path.join(ROOT, 'app', 'api', 'notifications', 'badge-count', 'route.ts')
...
const badgeCountContent = fs.existsSync(badgeCountRoutePath) ? fs.readFileSync(badgeCountRoutePath, 'utf-8') : ''
...
assert(fs.existsSync(badgeCountRoutePath), 'T18: badge-count route exists at app/api/notifications/badge-count/')

// T19: Badge count route exports GET
assert(badgeCountContent.includes('export') && badgeCountContent.includes('GET'), 'T19: badge-count route exports GET')

// T20: Badge count route uses createServerClient (AD-13)
assert(badgeCountContent.includes('createServerClient'), 'T20: badge-count route uses createServerClient (AD-13)')

// T21: Badge count route does NOT use createServiceClient (AD-13)
assert(!badgeCountContent.includes('createServiceClient'), 'T21: badge-count route does NOT use createServiceClient')

// T22: Badge count route checks seen_at IS NULL for unseen briefings
assert(badgeCountContent.includes('seen_at'), 'T22: badge-count route checks seen_at for unseen briefings')
...
assert(seenRouteContent.includes('export') && seenRouteContent.includes('POST'), 'T28: seen route exports POST')

// T29: Seen route uses createServerClient (AD-13)
assert(seenRouteContent.includes('createServerClient'), 'T29: seen route uses createServerClient (AD-13)')
...
const briefingPanelContent = fs.existsSync(briefingPanelPath) ? fs.readFileSync(briefingPanelPath, 'utf-8') : ''
...
assert(appShellContent.includes('prevModeRef'), 'T35: AppShell uses prevModeRef for mode transition tracking')
...
assert(
  briefingPanelContent.includes('/api/briefings/') && briefingPanelContent.includes("method: 'POST'"),
...
assert(briefingPanelContent.includes('onHighFlagCount'), 'T39: BriefingPanel still calls onHighFlagCount (preserved)')
...
assert(seenRouteContent.includes('404'), 'T42: seen route returns 404 if briefing not found for date')
...
assert(
  chatPanelContent.includes('setBannerDismissed(true)') || chatPanelContent.includes('setBannerDismissed'),
...
assert(
  chatPanelContent.includes('#f59e0b') || chatPanelContent.includes('amber') || chatPanelContent.includes('245,158,11'),
  'T54: ChatPanel banner uses amber urgency color',
)

// T55: Seen route does NOT use createServiceClient (AD-13)
assert(!seenRouteContent.includes('createServiceClient'), 'T55: seen route does NOT use createServiceClient (AD-13)')
...
assert(
  briefingPanelContent.includes('applyBriefing') && briefingPanelContent.includes('/api/briefings/'),
  'T58: BriefingPanel seen call is inside applyBriefing function',
)

// T59: package.json contains test:in-app-delivery51 script
assert(packageJsonContent.includes('test:in-app-delivery51'), 'T59: package.json contains test:in-app-delivery51 script')
