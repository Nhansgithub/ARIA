...
assert(
  evaluateTriggerCriteria(makeDeal({ stale_since: stale10DaysAgo }), {}, TODAY).includes(
    'stale_7d'
  ),
  'T2: stale_7d fires when stale_since 10 days ago'
)
assert(
  !evaluateTriggerCriteria(makeDeal({ stale_since: stale6DaysAgo }), {}, TODAY).includes(
    'stale_7d'
  ),
...
assert(
  !evaluateTriggerCriteria(
    makeDeal({ stale_since: stale10DaysAgo }),
    { stale_7d: last3DaysAgo },
    TODAY
  ).includes('stale_7d'),
...
assert(
  evaluateTriggerCriteria(
    makeDeal({ stale_since: stale10DaysAgo }),
    { stale_7d: last8DaysAgo },
    TODAY
  ).includes('stale_7d'),
  'T7: stale_7d fires when last check-in was 8 calendar days ago'
)
assert(
  evaluateTriggerCriteria(makeDeal({ next_action_due: tomorrow }), {}, TODAY).includes(
    'pre_action_due'
  ),
  'T8: pre_action_due fires when next_action_due is exactly tomorrow'
)
assert(
  !evaluateTriggerCriteria(makeDeal({ next_action_due: dayAfterTomorrow }), {}, TODAY).includes(
    'pre_action_due'
  ),
  'T9: pre_action_due does NOT fire when next_action_due is 2 days away'
)
assert(
  !evaluateTriggerCriteria(makeDeal({ next_action_due: TODAY }), {}, TODAY).includes(
    'pre_action_due'
  ),
  'T10: pre_action_due does NOT fire when next_action_due is today'
)
assert(
  !evaluateTriggerCriteria(makeDeal({ next_action_due: null }), {}, TODAY).includes(
    'pre_action_due'
  ),
...
assert(
  evaluateTriggerCriteria(
    makeDeal({ next_action_due: tomorrow }),
    { pre_action_due: lastYesterday },
    TODAY
  ).includes('pre_action_due'),
  'T12: pre_action_due fires when last check-in was yesterday (>= 1 calendar day)'
)
assert(
  evaluateTriggerCriteria(makeDeal({ next_action: 'Nhắc lần 1: gọi' }), {}, TODAY).includes(
    'cadence_followup'
  ),
  'T13: cadence_followup fires when next_action starts with Nhac lan 1'
)
assert(
  evaluateTriggerCriteria(makeDeal({ next_action: 'Nhắc lần 2: email' }), {}, TODAY).includes(
    'cadence_followup'
  ),
...
assert(
  !evaluateTriggerCriteria(
    makeDeal({ stale_since: null, next_action: 'Nhắc lần 1' }),
    {},
    TODAY
  ).includes('stale_7d'),
...
assert(
  !evaluateTriggerCriteria(makeDeal({ next_action_due: addDays(TODAY, 3) }), {}, TODAY).includes(
    'pre_action_due'
  ),
...
assert(
  !evaluateTriggerCriteria(makeDeal({ next_action_due: addDays(TODAY, -1) }), {}, TODAY).includes(
    'pre_action_due'
  ),
...
assert(
  evaluateTriggerCriteria(
    makeDeal({ next_action_due: tomorrow }),
    { pre_action_due: addDays(TODAY, -2) },
    TODAY
  ).includes('pre_action_due'),
...
const noPreDueResult = evaluateTriggerCriteria(
  makeDeal({ next_action_due: null, stale_since: stale10DaysAgo }),
  {},
  TODAY
)
assert(
...
assert(
  evaluateTriggerCriteria(makeDeal({ next_action: 'Nhắc lần' }), {}, TODAY).includes(
    'cadence_followup'
  ),
  'T43: next_action = bare Nhac lan prefix → fires'
)
assert(
  !evaluateTriggerCriteria(makeDeal({ next_action: 'nhắc lần 1' }), {}, TODAY).includes(
    'cadence_followup'
  ),
  'T44: lowercase n — case-sensitive prefix match → does NOT fire'
)
assert(
  !evaluateTriggerCriteria(makeDeal({ next_action: 'Follow up - Nhắc lần 1' }), {}, TODAY).includes(
    'cadence_followup'
  ),
...
assert(
  !evaluateTriggerCriteria(makeDeal({ next_action: 'Gọi lại' }), {}, TODAY).includes(
    'cadence_followup'
  ),
...
assert(
  !evaluateTriggerCriteria(
    makeDeal({ next_action: 'Nhắc lần 1' }),
    { cadence_followup: TODAY },
    TODAY
  ).includes('cadence_followup'),
  'T48: cadence_followup cooldown — last sent TODAY (same day) → does NOT fire'
)
assert(
  evaluateTriggerCriteria(
    makeDeal({ next_action: 'Nhắc lần 1' }),
    { cadence_followup: lastYesterday },
    TODAY
  ).includes('cadence_followup'),
  'T49: cadence_followup cooldown — last sent yesterday → fires'
)
const bothResult = evaluateTriggerCriteria(
  makeDeal({ stale_since: stale10DaysAgo, next_action: 'Nhắc lần 1' }),
  {},
  TODAY
)
assert(
...
if (serviceExists) {
  ...
  assert(
    service.includes('createServiceClient') && !service.includes('createServerClient'),
    'T53: uses createServiceClient not createServerClient'
  )
  assert(
    service.includes('export function evaluateTriggerCriteria'),
    'T54: exports evaluateTriggerCriteria'
  )
  assert(
    service.includes('export async function evaluateCheckInTriggers'),
    'T55: exports evaluateCheckInTriggers'
  )
  assert(service.includes('export async function scheduleCheckIn'), 'T56: exports scheduleCheckIn')
  assert(
    service.includes('export async function getPendingCheckIns'),
    'T57: exports getPendingCheckIns'
  )
  // Batch query — no N+1 per deal
  assert(
    service.includes('.in('),
    'T57b: uses batch .in() query for check_ins (not per-deal selects)'
  )
}
...
if (cronExists) {
  ...
  assert(
    !cron.includes('createServerClient'),
    'T60: cron route does NOT use createServerClient (AD-13)'
  )
...
const migrationPath = path.join(
  ...
  'supabase',
  'migrations',
  '20260630000000_check_ins_trigger_columns.sql'
)
