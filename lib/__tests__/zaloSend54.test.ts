...
test('T1: migration file exists', () =>
  assert(exists('supabase/migrations/20260701400000_zalo_delivery_status.sql'), 'missing'))
test('T2: adds zalo_status to briefings', () => {
  const lines = migrationDelivery.split('\n')
  assert(
    lines.some((l) => l.includes('briefings') || l.includes('ALTER TABLE briefings')),
    'briefings table missing'
  )
})
test('T3: adds zalo_status to check_ins', () => {
  const lines = migrationDelivery.split('\n')
  assert(
    lines.some((l) => l.includes('check_ins') || l.includes('ALTER TABLE check_ins')),
    'check_ins table missing'
  )
})
test('T4: zalo_status uses CHECK constraint with valid values', () =>
  assertIncludes(migrationDelivery, 'pending'))
test('T5: includes sent value in CHECK constraint', () => assertIncludes(migrationDelivery, 'sent'))
test('T6: includes failed value in CHECK constraint', () =>
  assertIncludes(migrationDelivery, 'failed'))
...
test('T9: migration file exists', () =>
  assert(exists('supabase/migrations/20260701410000_pg_cron_zalo_send.sql'), 'missing'))
test('T10: schedules send-zalo-notifications cron job', () =>
  assertIncludes(migrationCron, 'send-zalo-notifications'))
test('T11: fires at :05 UTC (before email at :15)', () =>
  assertIncludes(migrationCron, '5 * * * *'))
test('T12: calls /api/cron/send-zalo endpoint', () =>
  assertIncludes(migrationCron, '/api/cron/send-zalo'))
test('T13: uses cron_secret for authorization', () => assertIncludes(migrationCron, 'cron_secret'))
test('T14: unschedules previous job before re-scheduling', () =>
  assertIncludes(migrationCron, 'unschedule'))
...
test('T17: exports sendZaloMessage function', () =>
  assertIncludes(zaloService, 'export async function sendZaloMessage'))
test('T18: uses Zalo OA v2.0 message endpoint', () =>
  assertIncludes(zaloService, 'openapi.zalo.me/v2.0/oa/message'))
test('T19: sends access_token as header', () => assertIncludes(zaloService, "'access_token'"))
test('T20: uses AbortSignal.timeout (AD-6 timeout)', () =>
  assertIncludes(zaloService, 'AbortSignal.timeout'))
...
test('T23: checks Zalo error code 0 = success convention', () => {
  const code = codeLines(zaloService).join('\n')
  assert(
    code.includes('error !== 0') || code.includes('error && data.error !== 0'),
    'must handle Zalo 0=success convention'
  )
})
...
test('T27: exports formatBriefingForZalo function', () =>
  assertIncludes(briefingFormatter, 'export function formatBriefingForZalo'))
test('T28: hard limit at 2000 chars', () => assertIncludes(briefingFormatter, '2000'))
test('T29: truncation tail includes "Xem đầy đủ trong app ARIA"', () =>
  assertIncludes(briefingFormatter, 'Xem đầy đủ trong app ARIA'))
...
test('T34: interface BriefingForZalo is exported or defined', () =>
  assertIncludes(briefingFormatter, 'BriefingForZalo'))
...
test('T37: exports formatCheckInForZalo function', () =>
  assertIncludes(checkInFormatter, 'export function formatCheckInForZalo'))
test('T38: hard limit at 2000 chars', () => assertIncludes(checkInFormatter, '2000'))
test('T39: truncation tail includes "Xem đầy đủ trong app ARIA"', () =>
  assertIncludes(checkInFormatter, 'Xem đầy đủ trong app ARIA'))
...
test('T42: interface CheckInForZalo defined', () =>
  assertIncludes(checkInFormatter, 'CheckInForZalo'))
...
test('T49: filters briefings by zalo_status=pending', () =>
  assertIncludes(sendZaloRoute, "'pending'"))
test('T50: calls sendZaloMessage for briefings', () =>
  assertIncludes(sendZaloRoute, 'sendZaloMessage'))
...
test('T54: uses timing-safe CRON_SECRET comparison', () => {
  const code = codeLines(sendZaloRoute).join('\n')
  assert(
    code.includes('timingSafe') || code.includes('diff |=') || code.includes('timingSafeEqual'),
...
test('T56: GET handler returns response object', () =>
  assertIncludes(sendZaloRoute, 'export async function GET'))

// ─── T57-T62: webhook route ───────────────────────────────────────────────────
console.log('── app/api/webhooks/zalo/route.ts')

test('T57: webhook route file exists', () =>
  assert(exists('app/api/webhooks/zalo/route.ts'), 'missing'))
test('T58: exports GET for Zalo verification challenge', () =>
  assertIncludes(webhookRoute, 'export async function GET'))
test('T59: GET returns challenge param for verification', () =>
  assertIncludes(webhookRoute, 'challenge'))
test('T60: exports POST for inbound events', () =>
  assertIncludes(webhookRoute, 'export async function POST'))
...
test('T64: main test chain includes zaloSend54', () => {
  ...
  assert(
    mainTest !== undefined && mainTest.includes('zaloSend54'),
    'main test chain must include zaloSend54'
  )
})
