...
## Deferred from: code review of 4-1-pipeline-status-synthesis-stage-aware-next-action (2026-06-29)

- **`get_deal` sourced from DI_TOOLS carries DI-protocol description** — `lib/ai/pipelineStatusTools.ts:10`: The DI_TOOLS version of `get_deal` includes "Always call this first … load the full deal context before analysis" language from the Four-Layer Synthesis Protocol. No runtime impact today (same toolRunner handler), but if tools are later separated per intent, pipeline_status would silently lose the intelligence-field field descriptions. Address when tools are refactored per intent boundary.
- **Double `.sort()` on PIPELINE_STATUS_TOOLS** — `lib/ai/pipelineStatusTools.ts:15-20`: pipelineStatusTools sorts the array once at module load; agentWithTools.ts re-sorts on every request. The module-level sort is dead work. No functional issue; remove module-level `.sort()` call in a future cleanup pass.
- **`get_deal(title=<name>)` has no disambiguation protocol** — `lib/ai/orchestrator.ts pipeline_status Step 1b`: If multiple deals share a similar title, the lookup returns one silently. Pre-existing gap across all intents that use title-based lookup. Address as a cross-cutting CRM concern (disambiguation flow) in a future story.
